...
module Primitives = struct
  ...
  let echo pretend x y () =
    if not pretend then do_echo x y; (* no print here yet *)
    ""
end
...
let flatten_commands quiet pretend cmd =
  let rec loop acc =
    ...
    | Echo(texts, dest_path) :: xs ->
	(* why echo does the action even in "pretend" mode ? *)
	loop (Primitives.echo pretend texts dest_path :: acc) xs
    | Seq l :: xs -> loop (loop acc l) xs
  in List.rev (loop [] [cmd])



(* here starts the parallel world ! *)
...
let execute ?(quiet=false) ?(pretend=false) cmd =
  let cmds_supended = flatten_commands quiet pretend cmd in
  (* if we have to pretend, we pretend and stop there *)
  if pretend
  then begin
    List.iter (fun f -> ignore (f ())) cmds_supended;
    return ()
  end else begin
    (* when we don't pretend, well, we do it !*)
    let rec loop = function
      | [] -> return ()
      | cs :: css' ->
          let c = cs () in
          exec_scmd c >>= fun (out, err, stat) ->
            match stat with
            | WEXITED 0 ->
                if not quiet then
                  (display c; odisplay err; odisplay out);
                loop css'
            | _ -> fail_scmd c out err stat
    in
    loop cmds_supended
  end
...
(* iter f on all the tags of x, in parallel *)
let iter_tags_p f x =
  let rec spec x =
    match x with
    | N | A _ | Sh _ | P _ | Px _ | V _ | Quote _ -> return ()
    | S l -> iter_p spec l
    | T tags -> f tags
  in
  let rec cmd x =
